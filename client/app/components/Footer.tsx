    ...
    return (
        <footer className="footer bg-base-200/60 p-10">
            <form className="gap-6">
                ...
                <fieldset>
                    <label className="label-text" htmlFor="subscribeLetter">{t("subscribeNewsletter")}</label>
                    <div className="flex w-full flex-wrap gap-1 sm:flex-nowrap">
                        <input className="input input-sm" id="subscribeLetter" placeholder={t("emailPlaceholder")} type="email" />
                        <button className="btn btn-sm btn-primary" type="submit">{t("subscribe")}</button>
                    </div>
                </fieldset>
            </form>
            <nav className="text-base-content">
                <h6 className="footer-title">{t("footerServices")}</h6>
                <a href="#" className="link link-hover">{t("accounting")}</a>
                <a href="#" className="link link-hover">{t("taxPlanning")}</a>
                <a href="#" className="link link-hover">{t("payroll")}</a>
                <a href="#" className="link link-hover">{t("financialConsulting")}</a>
            </nav>
            <nav className="text-base-content">
                <h6 className="footer-title">{t("footerCompany")}</h6>
                <a href="#" className="link link-hover">{t("aboutUs")}</a>
                <a href="#" className="link link-hover">{t("contact")}</a>
                <a href="#" className="link link-hover">{t("careers")}</a>
                <a href="#" className="link link-hover">{t("pressKit")}</a>
            </nav>
            <nav className="text-base-content">
                <h6 className="footer-title">{t("footerLegal")}</h6>
                <a href="#" className="link link-hover">{t("termsOfUse")}</a>
                <a href="#" className="link link-hover">{t("privacyPolicy")}</a>
                <a href="#" className="link link-hover">{t("cookiePolicy")}</a>
            </nav>
        </footer>
    );
}